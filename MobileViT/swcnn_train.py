...
def run_one_epoch(
    ...
    if is_master() and train:
        for width_mult,_quantbits in train_tmp:
                ...
                with open('train_database.txt','a+') as f:
                    f.write('\n{:.1f}s\t{}\t{}\t{}/{}: '.format(time.time() - t_start, phase, str(width_mult)+'+'+str(_quantbits), epoch, FLAGS.num_epochs) + ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))
...
def test_one_epoch(
    ...
    if is_master():
        for _width_mult,_quantbits in test_all:
            ...
            with open('test_database.txt','a+') as f:
                f.write('\n{:.1f}s\t{}\t{}\t{}/{}: '.format(time.time() - t_start, phase, str(_width_mult)+'+'+str(_quantbits), epoch, FLAGS.num_epochs) + ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))
    else:
        results = None
    return results
...
def train_val_test():
    ...
    model_wrapper_teach.apply(lambda m: setattr(m, 'quant_bits', 255))
