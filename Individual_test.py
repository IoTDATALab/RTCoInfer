...
def test_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        for _width_mult,_quantbits in FLAGS.train_only_one:
            print(f'{batch_idx}/{len_loader} {_width_mult},{_quantbits}')
    ...
    if is_master():
        for _width_mult,_quantbits in FLAGS.train_only_one:
            ...
            print(results['top1_error'])
            with open('Individual_test_database.txt','a+') as f:
                f.write(str(_width_mult)+'+'+str(_quantbits)+' '+str(results['top1_error'])+'\n')
    else:
        results = None
    return results
