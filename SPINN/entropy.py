...
def forward_loss(
    ...
    if soft_target is not None:
        ...
        loss2 = torch.mean(criterion(output2, target))
    loss=0.15*loss2+loss
    # topk
    _, pred = output2.topk(max(FLAGS.topk))
    pred = pred.t()
...
def run_one_epoch(
    ...
    if getattr(FLAGS, 'universally_slimmable_training', False):
        max_width = FLAGS.width_mult_range[0]
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if train:
            ...
            meter = meters[str(1.0)]
            if getattr(FLAGS, 'slimmable_training', False):
                loss,z_num=forward_loss(model, criterion, input, target, meter)
                z_num_1+=z_num
                z_num_all+=1
            else:
                loss,z_num=forward_loss(model, criterion, input, target, meters)
#                z_num_1+=z_num
#                z_num_all+=1
    print(z_num_1,z_num_all)
