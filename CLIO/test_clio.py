...
def get_meters(phase):
    ...
    for _width in FLAGS.num_list:
        for _quant in FLAGS.quant_list:	
            meters[str(_width)+'+'+str(_quant)] = get_single_meter(phase, str(_width)+'+'+str(_quant))
    if phase == 'val':
...
def forward_loss_test_all(
        model, criterion, input, target, meter, width,quant,soft_target=None,
        soft_criterion=None, return_soft_target=False, return_acc=False):
    """forward model and return loss"""
    output = model(input,width=width,quant_bits=quant)
...
def test_one_epoch(
        epoch, loader, model, criterion, optimizer, meters, phase='val',soft_criterion=None
        ) :
    ...
    quant_bit_all=[1,2,4,8,32]
    quant_bit_width=[1,3,15,255,65535]
    for batch_idx, (input, target) in enumerate(loader):
        target = target.cuda(non_blocking=True)
        for _width_mult in FLAGS.num_list:
            for _quant in FLAGS.quant_list:
                if is_master():
                    meter = meters[str(_width_mult)+'+'+str(_quant)]
                else:
                    meter = None
                forward_loss_test_all(model, criterion, input, target, meter,_width_mult,_quant)
    if is_master():
        for _width_mult in FLAGS.num_list:
            for _quant in FLAGS.quant_list:
                results = flush_scalar_meters(meters[str(_width_mult)+'+'+str(_quant)])
                print('{:.1f}s\t{}\t{}\t{}/{}: '.format(time.time() - t_start, phase, str(_width_mult)+'+'+str(quant_bit_all[quant_bit_width.index(_quant)]), epoch, FLAGS.num_epochs) + ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))
    else:
        results = None
    return results
