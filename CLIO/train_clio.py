...
def get_model():
    ...
    model = model_lib.Model(num_classes = FLAGS.num_classes, split_point=FLAGS.split_point, widths=FLAGS.num_list)
...
def get_meters(phase):
    ...
    def get_single_meter(phase, suffix=''):
        ...
        return meters
    meters = {}
    meters[str(FLAGS.num_list[-1])+'+'+str(32)] = get_single_meter(phase, str(FLAGS.num_list[-1])+'+'+str(32))
    meters[str(FLAGS.num_list[-0])+'+'+str(32)] = get_single_meter(phase, str(FLAGS.num_list[0])+'+'+str(32))
    if phase == 'val':
...
def forward_loss(
        model, criterion, input, target, meter, width, soft_target=None,
        soft_criterion=None, return_soft_target=False, return_acc=False):
    """forward model and return loss"""
    output = model(input,width=width)
...
def run_one_epoch(
        epoch, loader, model, criterion, optimizer, meters,phase='train',soft_criterion=None
        ) :
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if train:
            ...
            optimizer.zero_grad()
            meter = meters[str(FLAGS.num_list[-1])+'+'+str(32)]
            for _width in FLAGS.num_list:
                loss = forward_loss(
                            model, criterion, input, target, meter,_width
                            )
                loss.backward()
                # if (getattr(FLAGS, 'distributed', False)
...
#            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=100, norm_type=1)
            ...
            if is_master():
                ...
                meter = None
            forward_loss_test(model, criterion, input, target, meter,width=FLAGS.num_list[-1])
    if is_master() and train:
        results = flush_scalar_meters(meters[str(FLAGS.num_list[-1])+'+'+str(32)])
        print('{:.1f}s\t{}\t{}\t{}/{}: '.format(time.time() - t_start, phase, str(FLAGS.num_list[-1])+'+'+str(32), epoch, FLAGS.num_epochs) + ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))
    elif is_master():
        results = flush_scalar_meters(meters[str(FLAGS.num_list[-1])+'+'+str(32)])
        print('{:.1f}s\t{}\t{}\t{}/{}: '.format(time.time() - t_start, phase, str(FLAGS.num_list[-1])+'+'+str(32), epoch, FLAGS.num_epochs) + ', '.join('{}: {:.3f}'.format(k, v) for k, v in results.items()))
    else:
