...
parser.add_argument('--drop', '--dropout', default=0, type=float, metavar='Dropout',
                    help='Dropout ratio')
parser.add_argument('--schedule', type=int, nargs='+', default=[80,160,240,320],
...
#        use_random_connect=args.use_random_connect,
    ...
    if args.resume:
        ...
        model_file = os.path.join(args.resume)
        checkpoint = torch.load(model_file)
        model.load_state_dict(checkpoint['state_dict'])
        # if args.change:
            # set_parameter_requires_grad(model, True)

        # model.load_state_dict(checkpoint_def['state_dict'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_checkpoint({
            ...
            }, is_best, checkpoint=dst, filename="ckpt.pth", info="model")
        if epoch%100==10:
            is_best=True
            save_checkpoint({
                'epoch': epoch + 1,
                ...
                'best_acc': best_acc,
                'optimizer' : optimizer.state_dict(),
            }, is_best, checkpoint=dst, filename=(str(epoch)+"ckpt.pth"), info="model")
...
# def adjust_learning_rate(optimizer, epoch):
	# global state
	# state['lr'] = args.lr*(args.epochs-epoch+1)/args.epochs
	# for param_group in optimizer.param_groups:
		# param_group['lr'] = state['lr']
