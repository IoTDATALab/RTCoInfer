...
def main():
    global best_acc, logfile,default_width,quant_bits
    ...
    print('zzzzzzzzzzzzzzzzzzz',model_file)
    checkpoint = torch.load(model_file)
    print('zzzzzzzzzzzzzzzzzzz',checkpoint['epoch'])
    ...
    epoch=0
    z_test=[]
    for quant_bits in [255]:
        for default_width in sorted([32,16,8,7,6,5,4,3,2,1],reverse=True):
...
def test(val_loader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(val_loader):
        ...
        outputs = model(inputs, width=default_width,quant_bits=quant_bits)
